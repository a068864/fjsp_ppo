...
Default training/eval is **demo-scale** (`5×3×4`, short PPO run). `--full-scale` is `25×15×8`, 2 097 152 steps, artifacts under `./checkpoints_full`. Measure FPS / RSS / GPU first:
...
| Checkpoints / TB | `./checkpoints`, `./logs` | `./checkpoints_full`, `./logs_full` |
...
Starts fresh by default on demo-scale. `--full-scale` writes `./checkpoints_full`, not the demo zips. Each run is **one point size** (`--n-machines` / `--n-jobs` / `--avg-ops` or a preset); size is not sampled on reset.
...
Written under `./checkpoints/` (demo) or `./checkpoints_full/` (`--full-scale`):
