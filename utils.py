...
def get_device(device: str = "auto") -> torch.device:
    """Select a torch device, preferring CUDA when available.
    ...
    if device == "auto":
        if torch.cuda.is_available():
            ...
            logging.getLogger(__name__).info("Using CUDA device: %s", torch.cuda.get_device_name(0))
            return resolved
        logging.getLogger(__name__).info("CUDA unavailable; falling back to CPU")
...
def set_global_seed(seed: int, deterministic: bool = True) -> None:
    """Seed Python, NumPy, PyTorch, CUDA, and Gymnasium for reproducibility.

    Args:
        seed: Global random seed.
        deterministic: If True, enable deterministic PyTorch algorithms where
            possible. Some CUDA ops may still warn or fall back.
    """
    seed = int(seed)
